...
- 修改原因：用户希望在不影响现有资金结算表佣金导出的前提下，新增基于佣金订单明细和招商订单明细的佣金报表，并可按豁免日期范围拆分全部/豁免/不豁免口径。
- 影响范围：仅影响香娜露儿（抖音）和慕莲蔓（抖音）页面新增的按明细表导出按钮及其新接口；不修改旧导出接口和旧按钮，不修改数据库结构，不写入业务数据。
- 是否涉及数据库：否（仅只读订单表、佣金订单明细、招商订单明细、豁免管理表）
- 是否需要回滚：是

## [2026-06-28 14:16] 修改记录
- 修改内容：调整香娜露儿/慕莲蔓抖音共用模块的佣金订单明细与招商订单明细导入防重配置。佣金订单明细必填字段由 `订单id/商品id/付款时间` 改为 `订单id/商品id/订单结算时间`，防重键由单一 `order_id` 改为 `order_id + product_id + settlement_time + commission_type`；招商订单明细必填字段由 `订单id/商品名称/付款时间` 改为 `订单id/商品id/结算时间`，防重键由单一 `order_id` 改为 `order_id + product_id + settlement_time + group_campaign_id`。
- 修改文件：douyin_shop_common/table_schemas.py；PROJECT_MEMORY.md
- 修改原因：用户确认佣金订单明细和招商订单明细导入防重不应只按订单号，应至少区分订单号和商品 ID，并进一步包含结算时间及佣金/团长活动维度，降低同一订单多商品、多结算或多佣金类型时误跳过数据的风险。
- 影响范围：仅影响香娜露儿（抖音）和慕莲蔓（抖音）后续新导入的佣金订单明细、招商订单明细预检与防重判断；不修改数据库表结构，不影响订单表、资金结算表导入，不影响现有旧佣金汇总/明细导出按钮和已有数据库记录。
- 是否涉及数据库：否（不改表结构、不直接写入业务数据；后续实际导入时会按新防重键写入新数据）
- 是否需要回滚：是
...
## [2026-06-18 13:56] 修改记录
- 修改内容：修复海外旗舰（抖音）佣金汇总/达人团长明细导出卡死失败问题。海外资金结算表不含达人名称时，原逻辑用资金表直接 `LEFT JOIN` 订单表并在关联条件中使用 `OR` 同时匹配子订单号和主订单号；在线上约 20 万资金行、28 万订单行的数据量下查询被放大并长时间无法返回。现改为先将订单表按子订单号、主订单号分别聚合成唯一达人映射，再分别关联资金表补齐达人昵称和达人ID，避免 `OR JOIN` 全表放大；同一优化同时应用于达人汇总和达人明细查询。
- 修改文件：douyin_shop_common/services.py；PROJECT_MEMORY.md
- 修改原因：用户反馈海外旗舰佣金导出失败，服务器复现发现导出请求长时间占用 CPU 无返回；手工优化 SQL 在服务器 2026 年 5 月真实数据上约 4.75 秒返回 77 个达人汇总。
- 影响范围：仅影响海外旗舰（抖音）佣金汇总导出和达人/团长明细导出中“从订单表补齐达人信息”的查询方式；香娜露儿、慕莲蔓标准抖音资金表本身含达人信息，导出逻辑不变；不改变导入、表结构、佣金金额口径和防重规则。
- 是否涉及数据库：否（只改查询 SQL，不改表结构和业务数据）
- 是否需要回滚：是

## [2026-06-18 14:01] 修改记录
- 修改内容：将海外旗舰（抖音）佣金导出 JOIN 修复提交并推送到 GitHub `main`，业务提交为 `80acf15`；随后在西班牙服务器 `208.85.17.83` 的 `/root/my-flask-project` 执行 `git pull --ff-only origin main`，从 `c3e40e3` 快进到 `80acf15`，并精准停止监听 `5001` 的旧 Flask 进程后使用 `.venv/bin/python` 重启服务。重启后服务器 HEAD 为 `80acf15`，进程 `763460` 监听 `0.0.0.0:5001`，服务器本机 `/auth/login` 返回 200，`/douyin_shop_overseas/` 未登录访问返回 302；真实 2026 年 5 月海外数据验证：佣金汇总 ZIP 2.42 秒生成，佣金明细 ZIP 39.16 秒生成。
- 修改文件：GitHub `main`；服务器 `/root/my-flask-project`；PROJECT_MEMORY.md
- 修改原因：用户要求检查佣金导出失败原因；确认并修复后需要让西班牙服务器运行最新代码。
- 影响范围：GitHub `main` 与西班牙服务器线上 Flask 应用代码及运行进程；未覆盖服务器 `data/` 业务数据，未处理服务器未跟踪的 `flask.log`。
...
## [2026-06-18 15:31] 修改记录
- 修改内容：检查豁免到期提醒逻辑并部署提醒中心功能。使用临时 SQLite 构造 7 天内到期、7 天外到期和已结束豁免记录，验证只广播“生效中且结束日期在提前天数窗口内”的记录，关闭豁免提醒后不再生成到期广播。随后将提醒中心与全站广播改动提交并推送到 GitHub `main`，业务提交为 `b3fd184`；西班牙服务器 `208.85.17.83` 的 `/root/my-flask-project` 已从 `3c9089d` 快进到 `b3fd184` 并重启 Flask，新进程 `767546` 监听 `0.0.0.0:5001`。服务器端模板解析通过，`/reminder_center/`、`/`、`/exemption_management/` 未登录访问均返回 302；服务器生产库提醒中心默认设置已初始化，广播总开关开启、豁免到期提醒开启、提前天数为 7，当前生产数据 7 天内暂无生效豁免到期提醒。
- 修改文件：GitHub `main`；服务器 `/root/my-flask-project`；PROJECT_MEMORY.md
- 修改原因：用户要求检查豁免提醒功能，并将所有更新推送 GitHub 后更新西班牙服务器运行新代码。
- 影响范围：GitHub `main` 与西班牙服务器线上 Flask 应用代码及运行进程；服务器新增提醒中心设置表，未覆盖 `data/` 业务数据，未处理服务器未跟踪的 `flask.log`。
- 是否涉及数据库：是
- 是否需要回滚：是

## [2026-06-19 10:40] 修改记录
- 修改内容：新增“快手美妆”模块（`kuaishou_meizhuang`），整体按快手澳柯模块写法接入订单表、资金流水表（结算表）和售后表三表导入、当前数据状态、原始数据筛选导出、佣金汇总导出和达人/团长明细导出。订单表与售后表沿用快手澳柯字段结构，订单表继续支持加密 Excel 使用文件名后 6 位密码读取；资金流水表按示例“跨境结算明细美妆”24 列独立适配，将 `结算时间` 作为状态/佣金筛选日期，将 `推广者佣金(元)`、`其他收费金额`、`人民币结算金额(元)` 等字段入库并参与达人佣金导出。上传仍使用 `common.upload_staging` 暂存批次机制，文件完整保存后再导入，成功/失败后清理暂存并记录日志，降低网速或请求中断导致半文件写库风险。已用示例文件在临时 SQLite 验证首次导入订单 3239 行、资金流水 13960 行、售后 405 行，同批第二次导入三表均写入 0 行，并验证佣金汇总 ZIP 可生成。
- 修改文件：kuaishou_meizhuang/__init__.py（新增）；kuaishou_meizhuang/table_schemas.py（新增）；kuaishou_meizhuang/services.py（新增）；kuaishou_meizhuang/routes.py（新增）；templates/kuaishou_meizhuang.html（新增）；app.py；auth/services.py；templates/index.html；PROJECT_MEMORY.md
- 修改原因：用户需要新增一个与快手澳柯功能和写法一致的快手美妆独立模块，用于导入快手美妆订单表、跨境结算资金流水表和售后表，并兼容美妆资金流水字段与快手澳柯不同的情况。
- 影响范围：新增快手美妆模块及其权限/首页入口；新增数据库表 `ks_meizhuang_orders`、`ks_meizhuang_fund_flow`、`ks_meizhuang_after_sales`、`ks_meizhuang_data_status`；不修改快手澳柯、抖音店铺、微信小店、发票、快递费等既有模块业务逻辑。
- 是否涉及数据库：是（首次访问/导入快手美妆模块时自动创建 3 张业务表和 1 张数据状态表；本次验证仅使用临时 SQLite，未写入本地 `data/main.db` 业务数据）
- 是否需要回滚：是

## [2026-06-19 11:18] 修改记录
- 修改内容：调整快手美妆佣金导出口径。资金流水表 `推广者佣金(元)` 仍按 `达人ID` 汇总到达人侧；`其他收费金额` 不再并入达人侧，而是在导出时按 `订单号 + 商品ID` 回查订单表 `团长ID/团长昵称`，匹配到团长时计入团长汇总和团长明细，匹配不到团长时进入未匹配佣金表。若同一订单在订单表中只对应一个团长，则允许用订单号兜底补齐团长 ID。页面说明同步改为“其他收费金额按订单表团长汇总”。已用快手美妆示例三表在临时 SQLite 验证：`其他收费金额` 合计 4859.90，其中 48.66 匹配到团长“紫烟海外优选”进入团长汇总，4811.24 因订单表范围不全或无团长匹配进入未匹配团长佣金；达人汇总不再包含其他收费金额。
- 修改文件：kuaishou_meizhuang/services.py；templates/kuaishou_meizhuang.html；PROJECT_MEMORY.md
- 修改原因：用户确认资金流水备注如 `商城分销信息服务费（授权达人推广)-17.32`，将该金额分配在达人侧不合理，应优先按订单表团长信息归属。
- 影响范围：仅影响快手美妆模块佣金汇总和达人/团长明细导出的归属口径；不修改三表导入、原始数据导出、数据库表结构、快手澳柯和其他模块。
- 是否涉及数据库：否（仅导出时读取现有订单表和资金流水表进行匹配；本次验证使用临时 SQLite）
- 是否需要回滚：是

## [2026-06-19 11:24] 修改记录
- 修改内容：调整快手澳柯佣金导出口径。资金流水表 `达人佣金(元)` 仍按 `达人ID` 汇总到达人侧，达人汇总和达人明细不再包含 `其他收费`；`团长佣金(元)` 与 `其他收费` 改为按 `团长ID/团长昵称` 汇总到团长侧，团长汇总新增 `其他收费` 与 `佣金合计` 列，应开金额导入和团长明细文件名按团长侧 `佣金合计` 取数。若资金流水中 `其他收费` 行缺少团长 ID，则导出时按 `订单号 + 商品ID` 回查订单表团长 ID；若同一订单在订单表只对应一个团长，则允许用订单号兜底补齐。已用临时 SQLite 构造验证：达人佣金 100 仅进入达人汇总；团长佣金 30 加其他收费 35 进入团长汇总，团长侧佣金合计为 65，应开金额总计为 165，团长明细文件名按 65.00 生成。
- 修改文件：kuaishou_aoke/services.py；templates/kuaishou_aoke.html；PROJECT_MEMORY.md
- 修改原因：用户要求快手澳柯与快手美妆保持同一归属口径，将 `商城分销信息服务费（授权达人推广)` 等其他费用归类到团长侧，而不是达人侧。
- 影响范围：仅影响快手澳柯模块佣金汇总和达人/团长明细导出的归属口径；不修改三表导入、原始数据导出、数据库表结构、快手美妆和其他模块。
- 是否涉及数据库：否（仅导出时读取现有订单表和资金流水表进行匹配；本次验证使用临时 SQLite）
- 是否需要回滚：是

## [2026-06-19 11:34] 修改记录
- 修改内容：将快手美妆新模块与快手澳柯/快手美妆佣金归属调整提交并推送到 GitHub `main`，业务提交为 `210707d`；随后在西班牙服务器 `208.85.17.83` 的 `/root/my-flask-project` 执行 `git pull --ff-only origin main`，从 `f71dd11` 快进到 `210707d`，并使用 `.venv/bin/python` 重启 Flask 服务。重启后新进程 `783626` 监听 `0.0.0.0:5001`，服务器本机 `/auth/login` 返回 200，`/kuaishou_aoke/` 与 `/kuaishou_meizhuang/` 未登录访问均返回 302 到登录页；公网 `http://208.85.17.83:5001/auth/login` 返回 200，`/kuaishou_meizhuang/` 返回 302 到登录页。
- 修改文件：GitHub `main`；服务器 `/root/my-flask-project`；PROJECT_MEMORY.md
- 修改原因：用户要求把本地修改上传 GitHub 仓库，并从仓库更新西班牙服务器运行代码。
- 影响范围：GitHub `main` 与西班牙服务器线上 Flask 应用代码及运行进程；服务器新增快手美妆模块代码并更新快手澳柯佣金导出口径，未覆盖服务器 `data/` 业务数据，未处理服务器未跟踪的 `flask.log`。
- 是否涉及数据库：否（代码部署；快手美妆数据表仍为首次访问/导入时自动创建）
- 是否需要回滚：是

## [2026-06-19 11:50] 修改记录
- 修改内容：将快手澳柯与快手美妆佣金导出口径恢复为“其他收费/其他收费金额归入达人侧”。移除导出时按 `订单号 + 商品ID` 回查订单表团长 ID 并把其他收费转入团长侧的逻辑；达人汇总和达人明细重新包含 `其他收费`，达人 `佣金合计 = 达人佣金 + 其他收费`；团长汇总和团长明细重新只统计资金流水表明确的 `团长佣金(元)`。页面说明同步恢复为其他收费归入达人侧。经快手美妆示例三表临时 SQLite 验证：达人汇总列为 `达人ID/达人昵称/达人佣金/其他收费/佣金合计`，其他收费进入达人侧；团长侧不再因其他收费产生未匹配。针对用户关心的其他收费转团长未匹配原因，统计发现美妆样表 `其他收费金额` 非 0 共 181 行、金额 4859.90，其中 163 行金额 4099.17 在当前订单表中找不到 `订单号 + 商品ID`，15 行金额 712.07 找到订单但订单表团长 ID 为空，3 行金额 48.66 可匹配团长；未发现“有团长 ID 但团长昵称为空”的情况。
- 修改文件：kuaishou_aoke/services.py；kuaishou_meizhuang/services.py；templates/kuaishou_aoke.html；templates/kuaishou_meizhuang.html；PROJECT_MEMORY.md
- 修改原因：用户反馈其他收费放到团长侧时未匹配情况过多，要求恢复为达人侧，并需要确认未匹配原因。
- 影响范围：仅影响快手澳柯与快手美妆佣金汇总、应开金额导入、达人/团长明细导出的归属口径和页面说明；不修改三表导入、原始数据导出、数据库表结构和其他模块。
- 是否涉及数据库：否（仅导出逻辑调整；本次验证使用临时 SQLite）
- 是否需要回滚：是

## [2026-06-19 13:52] 修改记录
- 修改内容：修复页头打印 WPS 今日记录解析与打印分页问题。WPS 解析中未匹配商品的自由文本不再从 `*1`、`1个` 等片段自动提取数量，改为保留原文且数量为 0，避免 `100ml*1` 这类文本在打印时被额外追加第二个 `*1`；页头打印弹窗固定为 `76mm × 130mm` 页面高度，并在调用打印前按内容实际高度自动缩放，避免内容过长时从“日升蓝盖绵羊油”等最后几行后继续换页。已用临时 SQLite 验证商品编号 `140` 仍解析为数量 1，自由文本 `100ml*1` 解析为数量 0；本次修改提交并推送到 GitHub `main` 后更新西班牙服务器运行代码。
- 修改文件：label_print/routes.py；templates/label_print.html；PROJECT_MEMORY.md
- 修改原因：用户反馈 WPS 解析自由文本 `100ml*1` 已按文本打印时仍被解析出数量导致打印重复 `*1`，同时实际打印页头时最后一行后发生换页，希望后续打印起始和结束位置固定。
- 影响范围：仅影响页头打印模块 WPS 自由文本解析结果和打印弹窗排版；不修改商品匹配、包装推荐算法、WPS 读取逻辑、打印历史表结构和其他业务模块。
- 是否涉及数据库：否
- 是否需要回滚：是

## [2026-06-19 14:00] 修改记录
- 修改内容：修正上一版页头打印分页处理方式。撤销打印前按内容高度自动缩放到单页的逻辑，恢复原有字号与行距；改为通过 `@page { size: 76mm 130mm; margin: 14mm 5mm 5mm; }` 为每一张打印页固定页头和页尾预留区域，内容超过当前页可打印区域时自然分页到下一页，下一页仍从同样的页头边界开始。保留 WPS 自由文本不自动解析数量的修复。
- 修改文件：templates/label_print.html；PROJECT_MEMORY.md
- 修改原因：用户确认实际需求不是把所有内容压缩进同一页，而是尊重每张纸的页头/页尾边界，超过页尾的后续内容应进入下一页。
- 影响范围：仅影响页头打印弹窗的分页 CSS；不修改 WPS 读取、WPS 解析商品数量、包装推荐、打印历史表结构和其他模块。
- 是否涉及数据库：否
- 是否需要回滚：是

## [2026-06-25 10:46] 修改记录
- 修改内容：调整页头打印 WPS 查看/解析中的自由文本数量处理。自由文本仍调用已有数量解析逻辑提取 `*1`、`2件` 等数量，并在解析结果表中保留该数量用于打印总数量统计；打印弹窗渲染自由文本时不再额外追加 `*数量`，避免原文已包含 `100ml*1` 时打印成重复数量。
- 修改文件：label_print/routes.py；templates/label_print.html；PROJECT_MEMORY.md
- 修改原因：用户反馈上一版将自由文本数量置空后会导致打印总数量少算，但直接显示解析数量又会让自由文本中的 `*1` 重复显示。
- 影响范围：仅影响页头打印模块 WPS 自由文本解析数量与自由文本打印显示；不修改产品编号解析、包装推荐、WPS 读取、打印历史表结构和其他业务模块。
- 是否涉及数据库：否
- 是否需要回滚：是

## [2026-06-25 10:59] 修改记录
- 修改内容：调整页头打印 WPS 解析结果中手工新增自由文本行的交互。点击“加自由文本”生成的手工自由文本行输入内容或数量时，不再实时同步刷新下方“货物录入”表，避免手工输入被货物编号录入逻辑干扰；手工自由文本保存 `manual` 标记，打印时允许显示手工填写的 `*数量`。自动解析出来的自由文本仍保持上一版逻辑：数量参与总数量统计，但打印时不额外显示解析数量。
- 修改文件：templates/label_print.html；PROJECT_MEMORY.md
- 修改原因：用户反馈手工新增自由文本时输入会联动下方货物录入，导致难以逐字填写；同时手工填写的件数应允许打印。
...
- 是否需要回滚：是
## [2026-06-26 16:05] 修改记录
- 修改内容：修复发票列表“匹配账单明细”弹窗的匹配后余数口径，由“发票金额 - 本发票匹配金额合计”改为“账单总额 - 本发票匹配金额合计”。发票列表达人/团长昵称改为可点击，只读弹窗展示该达人/团长所有应开账单；点击具体账单后展示该账单已匹配的所有发票。账单与发票合计按可用发票匹配金额比较，显示“账票相符 / 账单金额大于发票金额 / 发票金额大于账单金额”状态和颜色提示。
- 修改文件：invoicing/routes.py；templates/invoicing_invoices.html；PROJECT_MEMORY.md
- 修改原因：用户确认此前理解有歧义，需要从达人/团长维度查看应开账单，再从账单维度查看已匹配发票，并修复现有发票弹窗余数显示不符合账单核对口径的问题。
...
## [2026-07-03 11:06] 修改记录
- 修改内容：修复快手澳柯达人/团长明细导出在团长佣金区间报错的问题，将团长明细文件名金额来源从不存在的“佣金合计”列改为“团长佣金”列。已同步到马德里服务器 `208.85.17.83` 的 `/root/my-flask-project/kuaishou_aoke/services.py`，部署前备份为 `/root/backups/my-flask-project/manual-code-backups/kuaishou_aoke_services_before_detail_export_fix_20260703_030455.py`，并重启 Flask 进程。服务器生产库验证 `2026-06-01` 至 `2026-06-30` 汇总导出正常，明细导出生成 `快手澳柯佣金明细_2026年6月.zip`，大小约 369337 字节，ZIP 内 28 个文件；`/auth/login` 在公网 `5001` 与 `80` 均返回 200。
- 修改文件：`kuaishou_aoke/services.py`；服务器 `/root/my-flask-project/kuaishou_aoke/services.py`；`PROJECT_MEMORY.md`
- 修改原因：快手澳柯 2026 年 6 月存在团长佣金数据，明细导出代码读取团长汇总表不存在的“佣金合计”列，导致 `KeyError: '佣金合计'`，从而无法导出 6 月 1 日至 6 月 30 日明细。
- 影响范围：仅影响快手澳柯佣金明细导出的团长金额取值与文件名金额显示；不影响导入、汇总导出、数据库结构及其他模块。
- 是否涉及数据库：否
- 是否需要回滚：是

## [2026-07-03 11:35] 修改记录
- 修改内容：调整快手澳柯佣金汇总导出口径，团长汇总与达人汇总一致新增“其他收费”和“佣金合计”列，团长佣金合计按“团长佣金 + 其他收费”计算；应开金额导入中团长金额改为取含其他收费的佣金合计。已同步到马德里服务器 `208.85.17.83` 的 `/root/my-flask-project/kuaishou_aoke/services.py`，部署前备份为 `/root/backups/my-flask-project/manual-code-backups/kuaishou_aoke_services_before_leader_other_fee_20260703_033212.py`，并重启 Flask 进程。服务器生产库验证 `2026-06-01` 至 `2026-06-30` 团长汇总列为 `团长ID/团长昵称/团长佣金/其他收费/佣金合计`，团长其他收费合计 `578.28`，团长佣金合计 `15123.39`，佣金合计等于团长佣金加其他收费，应开金额导入中团长金额均匹配佣金合计；明细导出可正常生成。
- 修改文件：`kuaishou_aoke/services.py`；服务器 `/root/my-flask-project/kuaishou_aoke/services.py`；`PROJECT_MEMORY.md`
- 修改原因：快手澳柯团长侧也存在其他收费，且其他收费明细为商城分销信息服务费，需要在团长汇总和应开金额导入中纳入该费用。
- 影响范围：仅影响快手澳柯佣金汇总导出、应开金额导入及团长明细文件名金额显示；不影响导入、数据库结构、明细表内容和其它模块。
- 是否涉及数据库：否
- 是否需要回滚：是

## [2026-07-03 12:00] 修改记录
- 修改内容：经口径复核后恢复快手澳柯“商城分销信息服务费（授权达人推广）”统一归达人侧的佣金汇总逻辑，撤回上一条记录中将其他收费并入团长侧的改动。团长汇总恢复为仅包含 `团长ID/团长昵称/团长佣金`，应开金额导入中团长金额恢复为取 `团长佣金`，保留 `2026-07-03 11:06` 已修复的团长明细文件名金额取 `团长佣金` 以避免明细导出报错。已同步到马德里服务器 `208.85.17.83` 的 `/root/my-flask-project/kuaishou_aoke/services.py`，部署前备份为 `/root/backups/my-flask-project/manual-code-backups/kuaishou_aoke_services_before_restore_other_fee_creator_side_20260703_035932.py`，并重启 Flask 进程。服务器生产库验证 `2026-06-01` 至 `2026-06-30` 达人汇总包含 `其他收费/佣金合计` 且其他收费合计 `1709.92`，团长汇总不包含其他收费列、团长佣金合计 `14545.11`，应开金额导入中团长金额均匹配团长佣金，明细导出正常生成；公网 `5001` 与 `80` 的 `/auth/login` 均返回 200。
- 修改文件：`kuaishou_aoke/services.py`；服务器 `/root/my-flask-project/kuaishou_aoke/services.py`；`PROJECT_MEMORY.md`
- 修改原因：公开规则与线上资金流水字段分布均显示“商城分销信息服务费（授权达人推广）”更符合达人推广链路费用，且 2026 年 6 月所有其他收费记录均存在达人ID、达人佣金为 0，其中部分记录团长ID为空，故不应并入团长侧统计。
- 影响范围：仅影响快手澳柯佣金汇总导出、应开金额导入及团长明细文件名金额显示；不影响导入、数据库结构、明细表内容和其它模块。
- 是否涉及数据库：否
- 是否需要回滚：是
...
- 修改原因：用户需要在别名列表按指定平台账单周期查看佣金，且别名不是实际应开主体，必须按归类昵称/客户分别计算真实金额后再求和。
- 影响范围：仅影响达人/团长昵称管理页面的金额展示、周期筛选和关键词筛选后的顶部汇总；不修改数据库结构，不影响应开金额导入、佣金导出和发票匹配。
- 是否涉及数据库：否
- 是否需要回滚：是

## [2026-07-03 14:44] 修改记录
- 修改内容：达人/团长昵称管理页面继续补齐别名查询口径与发票查看弹窗。关键词 `q` 改为随“查询”一起提交到后端，昵称列表和别名列表都会先按关键词过滤再计算顶部汇总；切换昵称/别名标签和清空周期时保留已输入关键词。别名列表每个别名新增“发票”按钮，弹窗按当前别名和已选账单周期读取其下所有昵称匹配到的应开账单，未匹配发票的账单也会显示；弹窗汇总显示账单数、账单合计、可用发票合计、差额和别名，账单行显示账单ID、平台、期间、账期范围、开票主体、账单金额、可用发票合计、差额、状态、发票数，账单下方展开已匹配发票，发票号前增加账单ID，发票金额按该发票对本账单的可用匹配金额显示。
- 修改文件：`invoicing/routes.py`；`templates/invoicing_customers.html`；`PROJECT_MEMORY.md`
- 修改原因：用户要求查询时包含上方筛选条件，并希望每个别名单独查看当前账单周期内匹配过的发票，同时未匹配发票的账单仍要显示，发票金额使用可用发票金额。
- 影响范围：仅影响达人/团长昵称管理页面的后端查询、别名发票详情接口和前端弹窗展示；不修改数据库结构，不改变发票匹配保存逻辑、应开金额导入和佣金导出。
- 是否涉及数据库：否
- 是否需要回滚：是

## [2026-07-03 14:59] 修改记录
- 修改内容：调整达人/团长昵称管理页面别名发票弹窗界面，复用发票列表原有的应开账单弹窗风格和交互。弹窗恢复为先显示账单列表，点击某一账单行后在下方面板显示该账单已匹配发票；新增“显示全部/收起全部”按钮，点击“显示全部”后下方面板一次性展示当前别名、当前账单周期范围内所有账单的发票信息。下方面板的账单金额、可用发票合计、差额、发票数会随“选中单个账单/显示全部”实时重算，发票号继续保留“账单ID / 发票号”的显示方式。
- 修改文件：`templates/invoicing_customers.html`；`PROJECT_MEMORY.md`
- 修改原因：用户反馈新版弹窗界面不如发票列表原有实现，希望直接套用原发票列表的账单点击显示发票交互，并额外提供显示全部能力。
- 影响范围：仅影响达人/团长昵称管理页面别名发票弹窗的前端展示与交互；不修改数据库结构，不改变后端查询接口、发票匹配保存逻辑、应开金额导入和佣金导出。
- 是否涉及数据库：否
- 是否需要回滚：是

## [2026-07-03 15:18] 修改记录
- 修改内容：继续修正达人/团长昵称管理页面别名发票弹窗口径。后端别名发票接口新增返回 `invoice.alias_name = 当前别名` 的发票，用于覆盖发票解析确认入库和发票列表手工匹配时选择“匹配别名”的发票。前端下方面板不再为每条无匹配发票的账单显示“暂无已匹配发票”，只有当前展示范围内完全没有任何发票时才显示一次空提示；打开弹窗默认显示已匹配该别名的发票，点击账单时优先显示该账单对应发票，并用分割行继续显示其他匹配该别名但不属于当前账单的发票；点击“显示全部”时合并展示全部账单对应发票和其他别名发票。
- 修改文件：`invoicing/routes.py`；`templates/invoicing_customers.html`；`PROJECT_MEMORY.md`
- 修改原因：用户反馈“显示全部”时无匹配账单刷屏，并要求复核和补齐通过“匹配别名”入库/手工匹配的发票在别名弹窗中的展示。
