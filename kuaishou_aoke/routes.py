...
from flask import current_app, jsonify, render_template, render_template_string, request, send_file, url_for
...
def _run_store_self_sale_job(app, job_id: str, start_date: str, end_date: str) -> None:
    ...
    try:
        with app.app_context():
            filename = export_store_self_sale_zip_file(job['output_path'], start_date, end_date)
        with STORE_SELF_SALE_LOCK:
            job['status'] = 'complete'
            job['download_name'] = filename
...
@kuaishou_aoke_bp.route('/export_store_self_sale', methods=['POST'])
@module_required('kuaishou_aoke')
def export_store_self_sale():
    ...
    with STORE_SELF_SALE_LOCK:
        ...
        output_dir = Path(current_app.config['DATABASE_PATH']).resolve().parent / 'kuaishou_aoke_store_self_sale_jobs'
        ...
        STORE_SELF_SALE_JOBS[job_id] = {
            ...
            'message': '',
            'download_name': '',
            'output_path': str(output_dir / f'{job_id}.zip'),
        }
    ...
    return jsonify({
        'success': True,
        'job_id': job_id,
        'status_url': url_for('kuaishou_aoke.store_self_sale_job_status', job_id=job_id),
    }), 202


@kuaishou_aoke_bp.route('/export_store_self_sale/jobs/<job_id>')
@module_required('kuaishou_aoke')
def store_self_sale_job_status(job_id: str):
    with STORE_SELF_SALE_LOCK:
        ...
        return jsonify({
            ...
            'download_url': url_for('kuaishou_aoke.store_self_sale_job_download', job_id=job_id),
        })


@kuaishou_aoke_bp.route('/export_store_self_sale/jobs/<job_id>/download')
@module_required('kuaishou_aoke')
def store_self_sale_job_download(job_id: str):
    ...
    if not Path(path).is_file():
        return jsonify({'success': False, 'message': '导出文件不存在或已过期'}), 404
    return send_file(path, as_attachment=True, download_name=download_name, mimetype='application/zip')
